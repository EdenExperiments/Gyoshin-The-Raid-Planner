  ...
  if match:
    ...
    if isdatevalid == True:
      return sqlitedatetime
       ...
       await message.channel.send(f"Invalid datetime received, please beware you cannot search on dates in the past")
        
  # If it doesn't match the pattern throw an error
  else:
    invaliddatetimemessage = 'Invalid date and time entered, please make sure you enter it in the dd-mm-yyyy hh:mm format'
    await MessageHelper.PostMessage(message, invaliddatetimemessage)
